...
- **Extended `mlx-lm`** — vendored model modules (`scripts/mlx_models/`: DeepSeek V4, GLM-5 Next / GLM MoE DSA, Kimi K3, Qwen4-exp, Hy3/Hy4, Laguna, Inkling) plus runtime patches, installed on the nodes by the bootstrap script. Native MTP speculative decoding on supported families; per-model reasoning-tag handling (`reasoning_content` split).
