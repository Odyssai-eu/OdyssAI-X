...
# `TypeVar` way (rather than the 3.12 PEP-695 `def f[T](...)` syntax)
# so the file still parses under Python 3.11, which our cluster-node
# requirements pin (see requirements-node.txt).
...
from exo_stubs import ModelLoadingResponse
from exo_stubs import PipelineShardMetadata
from exo_stubs import logger
...
def patch_pipeline_model(model: _ModelT, group: mx.distributed.Group) -> _ModelT:
...
def patch_tensor_model(model: _ModelT) -> _ModelT:
