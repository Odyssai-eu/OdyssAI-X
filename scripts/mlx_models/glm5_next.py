# Copyright © 2026 Apple Inc. / Blaizzy (mlx-vlm) — Apache-2.0 / MIT lineage
#
# VENDORISÉ — adaptation mlx-lm du module glm5_next de mlx-vlm main
# (Blaizzy/mlx-vlm, mlx_vlm/models/glm5_next/, récupéré 2026-08-26 soir —
# l'implémentation MLX de référence pour zai-org/GLM-5.3-Flash).
# Transformation appliquée (documentée, reproductible) :
#   - corps du modèle (RMSNormGated → Glm5NextModel) repris VERBATIM de
#     language.py ; TextConfig → ModelArgs ; DeepseekMLP → DeepseekV32MLP
#     (signature identique, même lignée DeepSeek) ;
#   - imports re-basés sur mlx-lm : deepseek_v32 (MoE + sanitize MLA/fp8),
#     dsv4_hyper_connection (fichier IDENTIQUE ligne à ligne à celui de
#     mlx-vlm), gated_delta, mla.MultiLinear ;
#   - wrapper Model style mlx-lm ; sanitize = remap container
#     (model.language_model.* → model.*, vision droppée, mtp.* droppé) puis
#     le sanitize langage de la référence — qui inclut le DEQUANT FP8
#     BLOCKWISE via deepseek_v32.Model.sanitize (mx.from_fp8, blocs 128,
#     weight_scale_inv) : le checkpoint fp8 officiel se charge/convertit tel
#     quel, sans driver dédié.
# Si mlx-vlm ou mlx-lm publient une version amont, re-vendoriser et re-diff.
...
class ModelArgs(BaseModelArgs):
    ...
    def from_dict(cls, params):
        # Le config HF de GLM-5.3-Flash imbrique le texte sous text_config ;
        # le model_type au top-level (glm5_next) prime sur celui du nested.
        tc = params.get("text_config", params)
        merged = {**params, **tc}
        merged["model_type"] = params.get("model_type", merged.get("model_type"))
...
class Model(nn.Module):
    ...
    def sanitize(self, weights):
        # 1. Remap container VLM (référence glm5_next.py) : texte seul.
        remapped = {}
        for k, v in weights.items():
            ...
            if k.startswith("mtp.") or "mtp." in k.split("model.", 1)[-1][:5]:
                continue
            remapped[k] = v
        weights = {k: v for k, v in remapped.items() if "mtp." not in k}

        # 2. Sanitize langage de la référence (verbatim) — inclut dequant fp8
        #    blockwise + trim MTP + stack experts via deepseek_v32.
