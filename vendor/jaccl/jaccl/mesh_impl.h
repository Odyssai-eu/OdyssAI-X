...
class MeshImpl {
 public:
  ...
  void all_reduce(const T* in, T* out, int64_t size, ReduceOp reduce_op) {
    ...
    // Main loop
    ProgressGuard _pg1(liveness_fds_, rank_, "all_reduce"); // OdyssAI patch
    ...
    ProgressGuard _pg2(liveness_fds_, rank_, "all_reduce"); // OdyssAI patch
    while (in_flight > 0) {
      ibv_wc wc[WC_NUM];
      int n = poll(connections_, WC_NUM, wc);
      _pg2.tick(n > 0);
  ...
  void all_gather(const char* in_ptr, char* out_ptr, int64_t n_bytes) {
    ...
    ProgressGuard _pg3(liveness_fds_, rank_, "all_gather"); // OdyssAI patch
    while (in_flight > 0) {
      ibv_wc wc[WC_NUM];
      int n = poll(connections_, WC_NUM, wc);
      _pg3.tick(n > 0);
  ...
  void sum_scatter(const T* in, T* out, int64_t count, ReduceOp reduce_op) {
    ...
    //
    // Keep going until we have no longer data in flight.
    ProgressGuard _pg4(liveness_fds_, rank_, "sum_scatter"); // OdyssAI patch
    while (in_flight > 0) {
      ibv_wc wc[WC_NUM];
      int n = poll(connections_, WC_NUM, wc);
      _pg4.tick(n > 0);
  ...
  void send(const char* in_ptr, int64_t n_bytes, int dst) {
    ...
    // Main loop
    ProgressGuard _pg5(liveness_fds_, rank_, "send"); // OdyssAI patch
    while (in_flight > 0) {
      // Poll the hardware for completions.
      //
  ...
  void recv(char* out_ptr, int64_t n_bytes, int src) {
    ...
    // Main loop
    ProgressGuard _pg6(liveness_fds_, rank_, "recv"); // OdyssAI patch
    while (in_flight > 0) {
      // Poll the hardware for completions.
      //
 ...
 private:
  ...
  std::vector<int> liveness_fds_; // OdyssAI patch (PATCHES.md #1)
};
