...
// ── OdyssAI patch (PATCHES.md #1): liveness + progress guard ─────────────────
// UC queue pairs give no error completion when a peer dies (measured on Apple's
// Thunderbolt RDMA stack, macOS 26.6.1, scripts/jaccl/rdma_pair.c): a survivor
// polls forever. The side channel TCP sockets live as long as the group, so a
// closed socket IS the death signal. ProgressGuard is ticked from every
// completion wait loop; while no completion arrives it checks the side channel
// at most every 250 ms (poll()/MSG_PEEK only: no reads, no locks, safe from
// several wire threads at once) and enforces a hard progress timeout
// (JACCL_PROGRESS_TIMEOUT_S / MLX_JACCL_PROGRESS_TIMEOUT_S, default 600 s,
// 0 = disabled) as a backstop for a lost UC frame or a wedged peer.
...
class TCPAllGather {
 public:
  ...
  // OdyssAI patch: the socket fds (rank 0: one per peer in rank order; other
...
class SideChannel {
 public:
  ...
  // OdyssAI patch: same, with the side channel socket fds to watch (kept as a
  ...
  // OdyssAI patch: side channel socket fds to watch for peer death (empty when
